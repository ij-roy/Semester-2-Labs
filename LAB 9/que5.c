/*
Q5. A recipe is to be created for a cupcake. The ingredients are flour, butter, eggs, baking powder. All of these are quantities either as int or float. In addition, there is an additional ingredient called flavour which can be either choco-chip (int) or cream (float). 
a.	How will you capture flavour? Create the necessary data type for it.
b.	Create a structure for the cupcake. Take the details to make your cupcake from the user
c.	Create a function bake that prints the details. 
*/
#include <stdio.h>
